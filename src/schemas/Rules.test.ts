...
describe('Test rules', (): void => {
  it('return the appropritse messsage when account value surpasses threshold (with library)', async (): Promise<void> => {
    const rules = new Rules();
    const results = await rules.accountSurpassedThresholdLib({ name: 'acc1', value: 200 }, 200);
    expect(results.alertUser).toBe(true);
  });
